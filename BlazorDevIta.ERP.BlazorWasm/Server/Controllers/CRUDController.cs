...
namespace BlazorDevIta.ERP.BlazorWasm.Server.Controllers
{
        ...
        where EntityType : class, IEntity<IdType>, new()
    {
            ...
            [FromQuery] PageParameters parameters)
        {
            ...
            {
                //local void
                void Reset() { parameters.OrderBy = null; parameters.OrderByDirection = OrderDirection.Ascendent; }


                // ***First approach with action 👍 -------------------***

                result = result.OrderByProperty(parameters.OrderBy, parameters.OrderByDirection, Reset);
                ...
                // ***Second approach with wrapped object 🤔-------------------***

                //var orderedValues = result.GetOrderedValues(parameters.OrderBy, parameters.OrderByDirection);

                //if (!orderedValues.Error)
                //{
                //    result = orderedValues.value;
                //}
                //else Reset();


                // ***End Second approach with warpped object 🤔---------------***





                // ***Thrird approach Try and out parameter 🤔-------------------***

                //if (result.TryOrderByProperty(parameters.OrderBy, parameters.OrderByDirection, out IQueryable<EntityType> queryable))
                //{
                //    result = queryable;
                //}
                //else Reset();
                ...
                // ***End Thrird approach with warpped object 🤔-------------------***
